...
func (be *IceBackend) RegisterNetwork(ctx context.Context, wg sync.WaitGroup, config *subnet.Config) (backend.Network, error) {
	...
	if len(config.Backend) > 0 {
		if err := json.Unmarshal(config.Backend, &cfg); err != nil {
			return nil, fmt.Errorf("error decoding ICE backend config: %v", err)
		}
	}
	log.Infof("ICE config: VNI=%d Port=%d GBP=%v Learning=%v DirectRouting=%v", cfg.VNI, cfg.Port, cfg.GBP, cfg.Learning, cfg.DirectRouting)

	devAttrs := iceDeviceAttrs{
		vni:       uint32(cfg.VNI),
	...
	dev, err := newIceDevice(&devAttrs)
	if err != nil {
		return nil, err
	}
	dev.directRouting = cfg.DirectRouting

	data, err := json.Marshal(&iceLeaseAttrs{hardwareAddr(dev.MACAddr())})
	if err != nil {
		return nil, err
	}

	subnetAttrs := &subnet.LeaseAttrs{
